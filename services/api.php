	...
	class API extends REST {
		...
		*/
		...
		private function login(){
			...
			$error = array('status' => "Failed", "msg" => "Email ou Senha Inválido");
		...
		private function insertCustomer(){
			...
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Cliente cadastrado com sucesso.", "data" => $customer);
		...
		private function updateCustomer(){
			...
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Customer ".$id." Atualizado com sucesso.", "data" => $customer);
				$this->response($this->json($success),200);
			}else
				$this->response('',204);	// "No Content" status
		}
		...
		private function deleteCustomer(){
			...
			if($id > 0){				
				...
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Registro removido com sucesso.");
				$this->response($this->json($success),200);
			}else
				$this->response('',204);	// If no records "No Content" status
		}
